...
QueryBuilder.prototype.set = function(any, locals) {
  // check for timestamps and add them
  if (this.dao.table_options.timestamps === true) {
    // TODO:: Make this support camelCase format also
    if (_.contains(this.schema.columns, 'updated_at') ){
      any.updated_at = new Date();
    }
  }
...
QueryBuilder.prototype.insert = function(any, values) {
  ...
  if (this.dao.table_options.timestamps === true) {
    // TODO:: Make this support camelCase format also
    if (_.contains(this.schema.columns, 'created_at') ){
      any.created_at = new Date(); 
    }
  }
